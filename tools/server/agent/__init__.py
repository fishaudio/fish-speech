...
def execute_request(input_queue, tokenizer, config, request, device):
    ...
    yield from generate_responses(input_queue, tokenizer, config, request, prompt, im_end_id, device)
...
def get_response_generator(llama_queue, tokenizer, config, req, device, json_mode) -> partial:
    """
    ...
    if not req.streaming:
        ...
        return partial(streaming_generator, req, llama_queue, tokenizer, config, device, json_mode)