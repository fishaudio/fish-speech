...
def task_generator(config,filelist):
    ...
    for row in config["datasets"]:
        ...
        for name, subset in grouped_files.items():
            yield name, subset, source, languages, extension, filelist


def run_task(task):
    name, subset, source, languages, extension, filelist = task
    ...
    for file in subset:
        ...
        if np_file.exists() is False or txt_file.exists() and filelist is False:
