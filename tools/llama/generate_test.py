...
def decode_one_token_ar(
    ...
    **sampling_kwargs,
    ...
    codebooks[1:] = torch.masked_fill(
        codebooks[1:], codebooks[0] != 32003, CODEBOOK_PAD_TOKEN_ID
    )
...
def convert_string(input_str):
    semantic_count = input_str.count("<|semantic|>")
    output_str = input_str.split("<|semantic|>")[0].strip()
    output_str += f" [<audio of {semantic_count / 21.53:.2f} seconds>]"
    return output_str
    ...
    *,
    ...
    conv = Conversation(
        messages=[
            ...
            Message(role="user", parts=[np.load("example/q1.npy")]),
            Message(
                role="assistant",
                parts=["Transcribed:"],
            ),
    ...
    encoded_conv, _ = encode_conversation(
        conv,
        tokenizer,
        num_codebooks=model.config.num_codebooks,
    )
    encoded_conv = encoded_conv.to(device)
    encoded.append(encoded_conv[:, :-1])
    ...
    for sample_idx in range(num_samples):
        ...
        while seg_idx < len(encoded):
            ...
            code_mask = y[0, prompt_length:-2] == semantic_id
