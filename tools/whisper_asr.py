...
@click.option("--compute-type", default="float16", help="Computation Precision of the Whisper model [float16 / int8_float16 / int8]")
...
def main(model_size, compute_type, audio_dir, save_dir, sample_rate, device, language):
    ...
    model = WhisperModel(model_size, device=device, compute_type=compute_type, download_root="faster_whisper")
    ...
    for file_path in tqdm(audio_files, desc="Processing audio file"):
        ...
        for segment in segments:
            id, start, end, text = (segment.id, segment.start, segment.end, segment.text)
            ...
            transcript_save_path = save_path / rel_path.parent / f"{file_stem}-{id:03d}.lab"
            with open(
...
if __name__ == "__main__":
    ...
    audio = AudioSegment.from_wav(r"D:\PythonProject\原神语音中文\胡桃\vo_hutao_draw_appear.wav")

    model_size = "large-v3"

    model = WhisperModel(model_size, device="cuda", compute_type="float16", download_root="faster_whisper")
    ...
    for i, segment in enumerate(segments):
        print("Segment %03d [%.2fs -> %.2fs] %s" % (i, segment.start, segment.end, segment.text))
